        ...
        <div className="wrapper">
            {props.promotedProducts.map(product => {
                return(<div key={key++} className="announcement">
                    <div onClick={() => {props.gotoOffer(product.id)}} className="img-wrapper">                  
                        <img alt="no image available" src={product.images[1]}></img>
                    </div>
                    <span onClick={() => {props.gotoOffer(product.id)}} title={product.title} className="title"><span>{product.title.length <= 50 ? product.title : product.title.slice(0, 50) + "..."}</span></span>
